...
Primitive variables are reconstructed to the cell interfaces with a
piecewise-linear method using the monotonised-central limiter; the minmod
limiter and piecewise-constant reconstruction are available as alternatives
and are used for comparison. The scheme is second order in smooth regions
and requires two ghost zones. Reconstruction is performed on
$W v^i$ rather than on $v^i$, since limiting the three velocity components
independently can produce a superluminal interface state wherever the limiter
is active near a strong shear; density and pressure are additionally floored
by a piecewise-constant fallback wherever the linear reconstruction would make
them non-positive.
