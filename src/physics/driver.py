...
from .hlld        import (compute_srmhd_fluxes, hlld_flux, hlle_flux,
                          hllc_flux, primitive_to_conserved, hlld_ai_flux)
...
def _load_ai_solver(cfg: dict, device: str):
    ...
    stats_path = ml_cfg.get("norm_stats", "data/processed/default-mignone/norm_stats.npz")

    model = PressureNet(
        n_input    = ml_cfg.get("n_input", 15),
...
def run(cfg: dict):
    ...
    if problem == "generic":
        ...
        primL, primR, x_int = id_func(primL_cfg, primR_cfg, x_int,
                                       dtype=dtype, device=device, eos=eos)
    else:
    ...
    if solver == "hlld_ai":
        ...
        flux_fn = {"hlld": hlld_flux, "hlle": hlle_flux, "hllc": hllc_flux}.get(solver, hlld_flux)

    print(f"Starting {problem}  ncells={grid.ncells}  t_end={t_end}  solver={solver}  device={device}")

    while t < t_end:
        dt = compute_dt(prims, eos, grid.dx, cfl,
                        grid.ng, grid.ncells).item()
        dt = min(dt, t_end - t)
        if dt <= 0.0:
            break
        ...
        cons, prims = rk2_step(cons, prims, grid, eos,
                                torch.tensor(dt, dtype=dtype, device=device),
                                atmo_rho, flux_fn=flux_fn)
        t    += dt
        step += 1
